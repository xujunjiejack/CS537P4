  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != proc)
        continue;
      havekids = 1;
      if(p->state == ZOMBIE && p->is_thread == 0){
        // Found one.
        pid = p->pid;
        kfree(p->kstack);
        p->kstack = 0;
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != proc)
        continue;
      havekids = 1;
      if(p->state == ZOMBIE && p->is_thread != 0 && p->pgdir == proc->pgdir){
        // Found one.
        pid = p->pid;
        kfree(p->kstack);
        p->kstack = 0;
...
int clone(void (*fn)(void*), void* arg, void* ustack_base){
    ...
    // need to allocate stack for thread
    // Prepare for user stack. Set ustack_base
    uint sp;
    sp = (uint)ustack_base + PGSIZE - 1;

    // I assume there is only one argument
    int argc = 1;

    // Initialize stack
    uint ustack[MAXARG + 3+ 1];
...
// void (*leave)(void) = &exit;
    ustack[1] = (uint) arg;
//    ustack[0] = (uint) *leave;  // fake return PC
    //ustack[3] = 3;
